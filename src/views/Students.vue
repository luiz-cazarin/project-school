<template>
  ...
  <div class="students pa-6">
    <v-container>
      ...
      <v-btn text class="ma-2" elevation="2" @click="addStudent">
        Cadastrar
      </v-btn>
    </v-container>
    <!-- Lista de Alunos -->
    ...
    <v-expansion-panels>
      <v-expansion-panel v-for="student in students" :key="student.id">
        <v-expansion-panel-header>
          <v-list-item-title> Nome: {{ student.name }} </v-list-item-title>
          <v-list-item-subtitle>
            Matricula: {{ student.registration }}
          </v-list-item-subtitle>
          <v-btn icon max-width="40" plain @click="deleteStudent(student.id)">
...
export default {
  ...
  methods: {
    ...
    addStudent() {
      ...
      (this.newStudentName = ""), (this.newStudentRegistration = "");
